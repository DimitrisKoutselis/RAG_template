...
from src.infrastructure.external_services.langgraph.graph_nodes import GraphNodes
from src.infrastructure.external_services.langgraph.graph_state import RAGState


class LangGraphRAGEngine(IRAGEngine):
    """LangGraph-based RAG engine with stateful graph workflow."""
...
Always be accurate and cite specific information from the context when possible."""
    ...
    def __init__(
        ...
        self._nodes = GraphNodes(embedding_provider, vector_store, llm_provider)
    ...
    def _build_graph(self) -> StateGraph:
        ...
        workflow.add_node("embed_query", self._nodes.embed_query)
        workflow.add_node("retrieve", self._nodes.retrieve_chunks)
        workflow.add_node("build_context", self._nodes.build_context)
        workflow.add_node("generate", self._nodes.generate_response)
    ...
    async def process_query(
        ...
        initial_state: RAGState = {
            ...
            "conversation_history": (
                conversation.get_context_window() if conversation else []
            ),
            "top_k": config.top_k,
        ...
        if query.has_embedding():
            initial_state["query_embedding"] = query.embedding
    ...
    async def process_query_stream(
        ...
        """Process a query with streaming response using LangGraph."""
        config = config or RAGConfig()
